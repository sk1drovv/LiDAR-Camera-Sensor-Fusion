...
**Feature-level fúze** pracuje sekvenčně: shluky z LiDARu určují oblast zájmu a YOLOv8 klasifikuje, co se v ní nachází.
...
Naklonujte repozitář nebo rozbalte soubory projektu do libovolné složky, např.:

```
D:\Python\LiDAR-Camera-Sensor-Fusion\
```

Složka by měla obsahovat `README.md`, `requirements.txt` a adresář `src/` se všemi moduly Pythonu.
...
Rozbalte je do složky `DATASET/` tak, aby výsledná struktura odpovídala:
...
Dataset není součástí tohoto repozitáře. V souboru `src/config.py` nastavte `KITTI_DATA_PATH` a `KITTI_TRAINING_PATH` na cestu k vaší kopii KITTI.
...
Při dotazu na počet snímků zadejte `5` pro rychlý test. Po dokončení se uvnitř `src/` vytvoří složka `results/`, která by měla obsahovat:
...
| `CHYBA: Data KITTI nebyla nalezena` | zkontrolovat cestu `KITTI_DATA_PATH` v `src/config.py` |
...
Není součástí repozitáře:

```
DATASET/                                          # Dataset KITTI - nutno stáhnout samostatně
...
src/results/                   # Výstupní soubory, vytvořeny za běhu
...
Soubor `src/config.py` obsahuje všechny parametry systému:
